...
interface CreateExpenseModalProps extends Omit<ModalProps, 'children'> {
  id: string
  isOpen: boolean
  onClose: () => void
  onConfirm?: () => void
...
export default function CreateExpenseModal({
  ...
  const onSubmit: SubmitHandler<CreateCostSchema> = async (data) => {
    ...
    if (!id) {
      try {
        await addDoc(collection(db, 'expense'), formatData)
              ...
              <Controller
                ...
                render={({ field, fieldState: { error } }) => (
                  <Autocomplete
                    ...
                    label="Categoria"
                    variant="bordered"
                    defaultItems={EXPENSE_CATEGORIES}
              ...
              <Controller
                ...
                render={({ field, fieldState: { error } }) => (
                  <Autocomplete
                    ...
                    label="Sub-Categoria"
                    variant="bordered"
                    defaultItems={EXPENSE_CATEGORIES.filter(
                      (category) => category.id === watch().category
                    ).flatMap((category) => category.sub_category)}
                    placeholder="Pesquise pela sub-categoria"
                    errorMessage={error?.message}
                    endContent={<IconCategory className="text-xl" />}
                    onSelectionChange={(key) => field.onChange(Number(key))}
                    isRequired
